...
1. **Information Gathering**:
   ...
   - Use `src/components/visuals/infrastructure/LoadBalancingVisual.tsx` or `src/components/visuals/async/MessageQueuesVisual.tsx` as reference architecture.
...
3. **Implementation Steps**:
   - Create the new component in a categorized subdirectory: `src/components/visuals/[category]/[Topic]Visual.tsx`.
   - Create a corresponding CSS file in `src/styles/visuals/[category]/[topic].css` if custom diagram styles are needed.
   - Add the visual metadata to `src/data/visuals.ts`.
   - Ensure the visual is correctly exported and mapped in the main routing logic.
...
5. **Final Review**:
   ...
   - Ensure "Replay" functionality (in `VisualLayout`) of patterns works by resetting all animation state.
